...
const PaymentProcess: React.FC<PaymentProcessProps> = ({ onSubmit }) => {
  ...
  return (
    <div className="max-w-md p-6 mx-auto mt-10 bg-white rounded-lg shadow-md">
      {currentStep === "method-selection" && <PaymentMethodSelection onSelect={handleMethodSelect} />}
      {currentStep === "card-payment" && <CardPaymentForm onSubmit={handleCardPaymentSubmit} />}
    </div>
  );
};
